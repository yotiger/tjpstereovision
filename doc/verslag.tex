...
you can construct a graph where the labeling with the lowest energy is equal to the minimum cut on that graph. We will explain this
...
\paragraph{Labeling problem}
Determining the disparities can be considered as a labeling problem where every pixel is labeled with a disparity.
\paragraph{Graph theory: flow networks}
\begin{figure} \centering
\includegraphics[width=180px]{Network_flow.png}
\end{figure}

\paragraph{Graph theory applied to stereo vision}
\begin{figure}[t]
\centering
...
a chosen block of pixels $\mathbf{p}_{BT}$, where $d$ represents the disparity
in rectified images.\\ %, on subpixel level
%TODO check d
...
disparity changes\eqref{equ:enerform}.
...
and $\mathbf{q}$ in the base image $I_b$\eqref{equ:penality}.
...
The one with the lowest Energy cost is chosen. 
% TODO block or pixel? 
