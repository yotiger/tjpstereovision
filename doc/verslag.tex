...
occluded pixels, pixels that are found in one of the two images but
not in the other.
The algorithms we are trying to implement and use are namely:
\begin{itemize}
\item Graph Cut
\item Block Matching
...
algorithm. It is not the best algorithm for thi task but because of
his general usability and publicity it is a good algorithm to start
with. Furthermore we will test the implementations of the
standard block matching algorithm, which is faster but not that
precise. Also we will test a quite new implementation of the sub
global block matching algorithm, which was recently implemented in OpenCV.
...
\subsection{OpenCV}
\label{opencv} OpenCV is a library of programming functions for
computer vision. By ussing this library we can constrain our tasks to
integrating various parts of OpenCV and expanding it where
...
During our project we use the standard algorithms
that are already implemented in OpenCV. As described in the theory
part of the paper (see \ref{theory})we used three algorithms, graph
cut, block matching and semi global block matching. 
...
used the standard implementation in OpenCV which is described in
the Kolmogorov paper\cite{kolmogorov2003}. Our first results
almost reach the quality of the ground truth depth map provided by the Middlebury
...
Algorithm, but the quality of the results are not as good.
...
\ref{bm_comp} for comparison with the ground truth from
Middlebury. We couldn't find any good block matching example to
compare with but with the ground truth you will get the idea of how it
...
steps to get good results. The options that are tuneable are: 
\begin{itemize}
