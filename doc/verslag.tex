...
\section{Preface} 
Stereo vision is one of the key subjects in computer vision
...
that specific algorithm. It is quite slow, but because it is easy to use we will
start with this algorithm and later replace it with an implementation of block
matching also provided by OpenCV.
...
little as possible. \cite{kolmogorov2003} states that given some energy function
...
[REFNAARANDERGRAPHCUTPAPER]. For more details on the implementation in OpenCV,
...
achieve our goal. OpenCV is a C library but has Python bindings which
we used to decrease the risk of programming errors.
...
already implemented in OpenCV. As we have discussed previously, these algorithms
are:
\begin{itemize}
\item Graph Cut
\item Block Matching
...
maps is the industry standard for testing and comparing different methods of
stereo correspondence.

\subsubsection{Graph Cut}
The graph cut algorithm is one of the most popular algorithms in the
generation of disparity depth maps. We
used the standard implementation in OpenCV which is an adaptation of
...
homepage\cite{middlebury}. See figure \ref{gc_comp}.
Section \ref{gc_theory} talks about the theory behind graph cut.
...
\subsubsection{Block Matching} The Block Matching algorithm is much faster than
the Graph Cut Algorithm, but the quality of the results is not as good.  The
quality of the algorithm depends greatly on the configuration of the parameters
and the use of pre- and postfilters. See Figure \ref{bm_comp} for comparison
with the Graph Cut algorithm from Middlebury. We couldn't find any good block
matching example to compare with but with the graph cut you will get the idea of
how it has to divide the image into different depths.
...
steps to get good results. The parameters are:
...
all of our calculations. First off, it took us quite a while to get it working
on various systems. The library is written in C, but because we used Python
bindings, it was quite hard to debug errors thrown by the library functions.
