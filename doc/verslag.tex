...
To achieve this, a whole list of algorithms that do the
trick is available.\footnote{A good overview can be found at
...
The main goal of such an algorithm is matching one point in one
...
Another problem is occlusion of pixels. Some things that are
...
\ref{occprob} show that the leftmost pixel of the left cam is not visible
in the right cam view.
...
entire picture that disrupt those neighbouring relations as little as
...
\begin{figure} [h!tb]
  ...
  ``invalid'', that is using the algorithm no valid depth can }
  \label{gc_comp}
\end{figure}

\subsubsection{Block Matching}
The Block Matching algorithm is much faster than the Graph Cut
Algorithm, but the quality of the results until now are not as good.
